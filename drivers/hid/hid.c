...
/* Minimal USB HID shim.
 * For now we want the kernel to prefer the USB codepath for testing. The
 * real xHCI + HID implementation is TODO. To allow exercising the USB
 * branch without a full host stack, this shim advertises success when an
 * xHCI controller is present and forwards poll requests to the existing
 * PS/2 touchpad implementation. This keeps behavior identical while
 * letting the upper layers exercise the USB branch.
 */
