...
/* UEFI bootloader (moved from src/bootloader/bootloader.c)
 * - hardware init, GPT/FAT/ExFAT handling
 * - forced install logic, writes BOOTX64.EFI + ENTROPY.OS
 * - reads ENTROPY.OS from DATA (exFAT) partition and loads it via
...
{
    /* Route non-GUI text to serial (keep symbol for compatibility). */
    if (!s) return;
    serial_print(s);
}
...
static int bootloader_force_install(void)
{
    ...
    if (gpt_get_partition(0, &pi) == 0 && pi.valid) {
        ...
        if (fat32_format(fat_sectors, "BOOT") == 0 && fat32_init() == 0) {
            ...
            if (efi_cluster != 0) {
                ...
                if (boot_cluster == 0) {
                    uprint("Bootloader: failed to create/find EFI/BOOT directory\r\n");
                } else {
                    if (fat32_write_file_in(boot_cluster, "BOOTX64.EFI", BOOT_EFI, BOOT_EFI_SIZE) != 0) {
                        ...
                        /* Flush SATA write cache so FAT32 writes actually hit the disk */
                        if (sata_is_present()) {
                            for (int _f = 0; _f < 5; _f++) {
                                if (sata_flush_cache() != 0)
                                    uprint("Bootloader: warning — sata_flush_cache after BOOTX64.EFI failed\r\n");
                                for (volatile int _d = 0; _d < 100000000; _d++);
                            }
                        }
                        uprint("Bootloader: BOOTX64.EFI written and flushed to FAT32\r\n");
                    }
                }
            } else {
                uprint("Bootloader: failed to create/find EFI directory\r\n");
            }
        } else {
    ...
    if (gpt_get_partition(1, &pi_data) == 0 && pi_data.valid) {
        ...
        /* ExFAT format requires significant memory; use fallback if it fails */
            ...
            if (exfat_write_file("ENTROPY.OS", BOOT_EFI, BOOT_EFI_SIZE) == 0) {
                ...
                exfat_file_info_t vinfo; if (exfat_get_file_info("ENTROPY.OS", &vinfo) >= 0 && vinfo.valid) {
                    ...
                    if (exfat_create_dir_path("freedom/user") == 0) {
                        ...
                        /* Immediately flush and verify */
                        if (sata_is_present()) {
                            for (int _f = 0; _f < 5; _f++) {
                                if (sata_flush_cache() != 0) uprint("Bootloader: warning — sata_flush_cache after freedom/user failed\r\n");
                                for (volatile int _d = 0; _d < 100000000; _d++);
                            }
                        }
                    } else {
                    ...
                    if (exfat_create_dir_path("sys/DoNotTouch") == 0) {
                        ...
                        if (sata_is_present()) {
                            for (int _f = 0; _f < 5; _f++) {
                                if (sata_flush_cache() != 0) uprint("Bootloader: warning — sata_flush_cache after sys/DoNotTouch failed\r\n");
                                for (volatile int _d = 0; _d < 100000000; _d++);
                            }
                        }
                    } else {
                    ...
                    if (exfat_create_dir_path("sys/fonts") == 0) {
                        ...
                        if (sata_is_present()) {
                            for (int _f = 0; _f < 5; _f++) {
                                if (sata_flush_cache() != 0) uprint("Bootloader: warning — sata_flush_cache after sys/fonts failed\r\n");
                                for (volatile int _d = 0; _d < 100000000; _d++);
                            }
                        }
                    } else {
                    ...
                    /* Final flush before proceeding with file operations */
                    if (sata_is_present()) {
                        if (sata_flush_cache() != 0) uprint("Bootloader: warning — sata_flush_cache before file writes failed\r\n");
                    }
                    ...
                    /* Retry writes with post-write verification to match installer behaviour. */
                    ...
                    if (!write_ok) {
                        uprint("Bootloader: Failed to write /sys/DoNotTouch/Entropy.OS after retries\r\n");
                    }
                    ...
                    /* Force disk flush before fsck to ensure all writes are persisted */
                    if (sata_is_present()) {
                        if (sata_flush_cache() != 0) uprint("Bootloader: warning — sata_flush_cache before fsck failed\r\n");
                    }
                    
                    /* Small delay to ensure cache operations complete */
                    for (volatile int _delay = 0; _delay < 100000000; _delay++);
                    ...
                    /* Final disk flush before completion signal */
                    if (sata_is_present()) {
                        if (sata_flush_cache() != 0) uprint("Bootloader: warning — final sata_flush_cache failed\r\n");
                    }
                    
                    /* Another delay to ensure final flush completes */
                    for (volatile int _delay = 0; _delay < 100000000; _delay++);
    ...
    /* Give system time to flush everything before exiting */
    for (volatile int _final_delay = 0; _final_delay < 500000000; _final_delay++);
    
    /* Attempt graceful ACPI shutdown to allow QEMU to persist changes */
    /* This is optional; if it doesn't work, the bootloader just returns normally */
    if (gST && gST->RuntimeServices) {
        typedef EFI_STATUS (EFIAPI *efi_reset_system_t)(EFI_RESET_TYPE, EFI_STATUS, UINTN, VOID*);
        efi_reset_system_t reset = (efi_reset_system_t)gST->RuntimeServices->ResetSystem;
        if (reset) {
