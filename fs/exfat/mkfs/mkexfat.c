...
/* Forward declarations for objects that may come from compat.c in UEFI builds */
#ifdef __UEFI__
...
static int erase(struct exfat_dev* dev)
{
	const struct fs_object** pp;
	off_t position = 0;
	const off_t block_size = 4 * 1024;  /* 4 KB for bootloader with limited memory */
	void* block = malloc(block_size);

	if (block == NULL)
	{
		exfat_error("failed to allocate erase block of %"PRId64" bytes", block_size);
		return 1;
	}
...
int mkfs(struct exfat_dev* dev, off_t volume_size)
{
	...
	fflush(stdout);
#else
	exfat_debug("mkfs: erasing filesystem structure");
#endif
	if (erase(dev) != 0)
		return 1;
#ifdef __UEFI__
	exfat_heap_report();  /* Report memory usage after erase */
#endif
	if (create(dev) != 0)
		return 1;
#ifndef __UEFI__
